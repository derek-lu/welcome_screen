 ...
 * @param {Boolean} asEntitled The boolean to determine if we should set direct entitlement when fetch folio information.
 * @param {Dictionary} subscriberInfo The subscriberInfo could be null if user does not want to provide subscriber information.
 ...
 */
LibraryService.prototype._internalGetFolioInfo = function (productId, asEntitled, subscriberInfo) {
    
	var trans = new FolioInfoTransaction(productId, asEntitled, subscriberInfo);
	if (trans) {
		TransactionManager.instance._registerTransaction(trans);
...
/**
 * Start a Folio Info transaction without setting direct entitlement.
 * @memberOf adobeDPS-LibraryService.prototype
 * @param {String} productId The productId of the folio to fetch information.
 * @returns {adobeDPS-FolioInfoTransaction} The started folio info transaction.
 * @throws {Error} If there is already an active folio info transaction.
...
LibraryService.prototype.getFolioInfo = function (productId) {
    ...
    var asEntitled = false;
	return this._internalGetFolioInfo(productId, asEntitled, null);
};

/**
 * Start a Folio Info transaction with setting direct entitlement.
 * Entitlement will only be added to the folio if aggregatorMode is ON and 
 * user is signed in and folio is not free or not entitled;
 * otherwise, it will behave the same as GetFolioInfo.
 * @memberOf adobeDPS-LibraryService.prototype
 * @param {String} productId The productId of the folio to fetch information.
...
LibraryService.prototype.getFolioInfoAsEntitled = function (productId, subscriberInfo) {
    ...
    var asEntitled = true;
    return this._internalGetFolioInfo(productId, asEntitled, subscriberInfo);
};
 ...
 * @extends adobeDPS-Transaction
 */
function FolioInfoTransaction(productId, asEntitled, subscriberInfo) {
    ...
    /**
	 * The asEntitled determines whether we need to set direct entitlement or not when fetch folio information.
	 * @memberOf adobeDPS-FolioInfoTransaction.prototype
	 * @type Boolean
	 */
    this.asEntitled = asEntitled;
    
    /**
     * The subscriberInfo is a dictionary object. It is optional and current supported fields are subscriberID and subscriberType.
     * The subscriberInfo will be added to the entitlement if asEntitled is set as True and we are able to add entitlement
     * when fetch folio information. The subscriberInfo is only attached to the downloaded folio.
     * @memberOf adobeDPS-FolioInfoTransaction.prototype
     * @type Dictionary
     */
...
FolioInfoTransaction.prototype._toJSON = function () {
	var json = Transaction.prototype._toJSON.call(this);
	json.productId = this.productId;
    json.asEntitled = this.asEntitled;
    json.subscriberInfo = this.subscriberInfo;
...
function DeviceService() {
    ...
    /**
     * Indicates whether ad tracking is enabled. If false, the user has limited ad tracking then advertisingIdentifier
     * should only be used for frequency capping, conversion events, estimating the number of unique users, security
     * and fraud detection, and debugging.
     * @memberOf adobeDPS-DeviceService.prototype
     * @type Boolean
     */
